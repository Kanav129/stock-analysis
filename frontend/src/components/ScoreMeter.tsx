...
export function ScoreMeter({ value, size = 'sm', showLabel = true }: Props) {
  ...
  return (
    <div className={`flex items-center ${tall ? 'gap-3' : 'gap-2'} min-w-0`}>
      <div
      ...
      >
        ...
        {/* Score marker */}
        <div
          className={`absolute top-1/2 -translate-y-1/2 -translate-x-1/2 rounded-full border border-[var(--color-surface-0)] bg-[var(--color-text-primary)] shadow-sm transition-[left] duration-500 ${
      ...
      {showLabel && (
        <span
          className={`shrink-0 font-mono tabular-nums ${
            tall ? 'text-sm font-semibold' : 'text-xs'
          }`}
          style={{
            color:
              score > 15
